...
def matrix_iof(target_boxes, rois):
    """
    return iof of a and b, numpy version for data augenmentation
    """
    left_top = np.maximum(target_boxes[:, np.newaxis, :2], rois[:, :2])
    right_btm = np.minimum(target_boxes[:, np.newaxis, 2:], rois[:, 2:])

    area_intersection = np.prod(right_btm - left_top, axis=2) * (left_top < right_btm).all(axis=2)
    area_foreground = np.prod(target_boxes[:, 2:] - target_boxes[:, :2], axis=1)
...
def match(threshold, bboxes, priors, variances, labels, landms, loc_t, conf_t, landm_t, idx):
    ...
    Args:
        ...
        idx: (int) ~current batch index~ current index within a batch
    Return:
    ...
    # ignore hard gt, 0.2 servers as iou lower bound threshold
    ...
    # 2 targets (therefore two overlaps) per each anchor
